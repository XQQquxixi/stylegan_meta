...
class LossModel:
    def __init__(self, img_size, scale_reg=0.02, layer=9, batch_size=1, sess=None):
    ...
    def optimize(self, gen, ref, vars_to_optimize, iterations=500, learning_rate=1.):
        # forward
        gen_img = preprocess_input(tf.image.resize_images(gen, (self.img_size, self.img_size), method=1))
        generated_img_features = self.perceptual_model(gen_img)
        self.ref_img_features = tf.get_variable('ref_img_features', shape=generated_img_features.shape,
                                                dtype='float32', initializer=tf.initializers.zeros())
                                                self.features_weight = tf.get_variable('features_weight', shape=generated_img_features.shape,
                                                                                       dtype='float32', initializer=tf.initializers.zeros())
                                                self.set_reference_images(ref)
                                                
                                                self.sess.run([self.features_weight.initializer, self.features_weight.initializer])
                                                
                                                self.loss = tf.losses.mean_squared_error(self.features_weight * self.ref_img_features,
                                                                                         self.features_weight * generated_img_features) / 82890.0+tf.losses.mean_squared_error(gen, ref)
...
"""
    ...
    def build_loss_model(self, generated_image_tensor):
    vgg16 = VGG16(include_top=False, input_shape=(self.img_size, self.img_size, 3))
    self.perceptual_model = Model(vgg16.input, vgg16.layers[self.layer].output)
    ...
    self.ref_img_features = tf.get_variable('ref_img_features', shape=generated_img_features.shape, dtype='float32',initializer=tf.initializers.zeros())
    self.features_weight = tf.get_variable('features_weight', shape=generated_img_features.shape, dtype='float32', initializer=tf.initializers.zeros())
    self.sess.run([self.features_weight.initializer, self.features_weight.initializer])
    self.gen = generated_image_tensor
    ...
    def set_reference_images(self, images_list):
    assert(len(images_list) != 0 and len(images_list) <= self.batch_size)
    self.ref_img = load_images(images_list, self.img_size)
    self.loss += tf.losses.mean_squared_error(self.gen, self.ref_img)
    loaded_image = preprocess_input(self.ref_img)
    image_features = self.perceptual_model.predict_on_batch(loaded_image)
